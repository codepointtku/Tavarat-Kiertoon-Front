...
function EditProduct() {
    ...
    const {
        ...
        defaultValues: {
            ...
            // available: product?.product_items[0]?.available || false,
            barcode: product?.product_items[0]?.barcode,
            storages: product?.product_items[0]?.storage.id.toString(),
            shelf_id: product?.product_items[0]?.shelf_id,
            // amount: product?.amount, // amount will not be changed in this view?
            price: product?.price,
    ...
    return (
        <>
            {actionData?.status === true && (
                <AlertBox
                    status="success"
                    text="Tuotteen muokkaus onnistui"
                    redirectUrl="/varasto/tuotteet/"
                    timer={1000}
                />
            )}
            {actionData?.status === false && (
                <AlertBox status="error" text="Tuotteen muokkaus epäonnistui" timer={3000} />
            )}
            <Modal
                component={Box}
                open={qrScanOpen}
